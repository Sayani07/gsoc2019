...
data_MI %>% ggplot(aes(x = total_runs, y= as.factor(semester))) +  geom_density_ridges2()+ xlab("semester per match")+ ylab(" Total runs per ball")+ ggtitle("Ridge plot of runs per ball across each semesters of a match")
...
```
Notice that we could analyse runs per ball across balls, overs, quaters and semester of each match since we have data for 80 matches. So for each match balls, overs , quarters and semester can be thought to align linearly where balls vary from 1,2, ..120, overs vary from 1, 2, ...20, quarters range from 1, 2,...,4 and so on. We call these "linear" categorisation. We can also be interested to study the distribution of runs/ball across "circular" categorisations like balls per over/quarter/semester, overs per quarter/semester and quarters per semester. They are not "linear" since they keep on repeating themselves within a match. 
