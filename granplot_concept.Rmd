...
Assumptions are made to ensure display is not too cluttered and the space occupied by various kinds of distribution plots
   ...
   - medium (facet), medium (x -axis) -> c("ridge", "violin", "lv", "density")
...
**Warnings for granularities  hour_day and month_year
```{r}
data1 = tsibbledata::nyc_bikes %>% as_tsibble()
data2 = tsibbledata::aus_elec %>% as_tsibble()
gravitas:::gran_advice(data1, gran1="hour_day", gran2 = "month_year")
```
**Warnings for granularities  hour_day and day_week
```{r}
gravitas:::gran_advice(data2, gran1="hour_day", gran2 = "day_week")
```

**granplot
```{r}
gravitas::granplot(data2, ugran="week", lgran = "hour", response="Demand")
```
