...
Exploratory Data Analysis (EDA) through visualization and creating useful summaries at finer or coarser temporal scales can help with the analysis of probability distribution of smart meter data. In fact,  analysis of any temporal data that needs to be examined at several temporal resolutions can benefit from this approach.


This project would provide a mechanism to operate on time in an automated way, to deconstruct it in many different ways. Deconstruction that capture calendar effects like hour of the day or day of the month and accommodate for periodicities in time are defined as circular granularities/ calendar categorizations. 


# Related work

- Lubridate is an R package that makes it easier to work with dates and times and also has functions for creating calendar categorizations like hour of the day, day of the week, minutes of the hour. But it mostly creates calendar categorizations that are one step up. The framework proposed will allow creating calendar categorizations that are more than one step ahead, for example,  hour of the week or one step up that are not present in lubridate package like week of the month.
...
The package would facilitate a tidy workflow for exploring probability distributions of variables defined on temporal granularties. This will use as inputs tsibble objects which complement the tibble and extend the tidyverse concept to temporal data.
