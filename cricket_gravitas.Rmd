...
```{r build_gran}
dynamic_build_gran <- function(x, hierarchy_tbl = NULL, lower_gran = NULL, upper_gran = NULL, label = TRUE, abbr = TRUE, ...) {
  ...
  if (g_order(hierarchy_tbl, lower_gran, upper_gran) < 0) {
    ...
    if (g_order(hierarchy_tbl, lower_gran, upper_gran) == 1) {
      ...
      value <- one_order(.data, hierarchy_tbl, lower_gran, col_name = "balls_game")
    ...
    lev <- unique(value)
     ...
     if (label) {
    if (lower_gran == "day" & upper_gran == "week") {
      names <- c(
        "Sunday", "Monday", "Tuesday", "Wednesday",
        "Thursday", "Friday", "Saturday"
      )
    }
    else if (lower_gran == "month" & upper_gran == "year") {
      names <- c(
  ...
  value_label <- factor(value, labels = names_gran)
