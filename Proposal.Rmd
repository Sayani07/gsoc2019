...
Since the data fed is a tsibble, it will automatically feed the available time index in the function. These functions will be able to create granularities in combination with any time index that are one or multiple level higher in resolution than the available time index in the given tsibble.


Additionally, it should also be able to provide functions to create granulartities in combination with temporal scales that are one or multiple levels above half-hour like hour, day, week or month. 


| Function                         	| Description                                                                                                                           	|       Arg 1      	| Arg 2 	|
|----------------------------------	|---------------------------------------------------------------------------------------------------------------------------------------	|:----------------:	|------:	|
| derive_ghour (.data, .grantype1) 	| define granularities in combination with hour, e.g. hour of the week, when tsibble index is half hour or quarter hour                 	| a tsibble object 	|  week 	|
| derive_gweek (.data, .grantype1) 	| define granularities in combination with week, e.g. week of the month, when tsibble index can be day, hour, half hour or quarter hour 	| a tsibble object 	| month 	|
